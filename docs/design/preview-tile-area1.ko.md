...
| 팔레트        | 16 개 항목                          |
| 타일 키       | 12 키 (정적 11 + 애니메이션 1) — v0.50.2 가 `mile_4` 추가 |
...
- **`mile_1`**, **`mile_2`**, **`mile_3`**, **`mile_4`** — release-master 결정 Q4 에 따라 라운드-숫자는 **숫자** 로 (notch mark 가 아님) 베이크된다. 각 타일은 풍화된 나무 기둥 (wet-bark-brown 축 + cuff-cream 판자 가로대) 을 보여준다. 판자 면이 보랏 외곽선에 숫자를 담는다:
  ...
  - **`mile_4`** *(v0.50.2)*: 좌상단 수직 + 우상단 수직 + 그것들을 잇는 중간 수평 + 중간 아래로 내려가는 우하단 수직. 클래식 7-세그먼트 "4" — 상단 양쪽에 측당 세 개의 잉크 셀, 그 다음 한-셀 수평 조이너, 그 다음 우측에 한 컬럼의 잉크가 내려간다. "4" 로 읽힌다.
  ...
  > **v0.50.2 mile-marker 이동.** v0.50.1 은 `mile_1` / `_2` / `_3` 을 라운드들 사이에 배치했다 (Round 1 후 = Round 2 마커, 등). v0.50.2 는 마커를 라운드 START 로 이동 — Round 1 마커는 스테이지 입구에, Round 2 마커는 Round 2 가 시작하는 스크린에, Round 3 마커는 Round 3 에, Round 4 마커는 Round 4 에. 그래서 새로운 `mile_4` 타일: 이제 네 개의 시작-마커와 하나의 boundary cairn 이 있다. Round 4 끝의 cairn (스테이지-클리어 경계에서) 은 v0.50.2 에서 아트 변경 없음.

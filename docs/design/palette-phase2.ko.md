...
## v0.50.2 보충 — 스프린트 트레일 wisp

v0.50.2 패치는 새로운 `sprint` / `sprint_armed` 모션-라인 트레일을 위해
`assets/sprites/hero-reed.js` 에 두 개의 팔레트 항목을 추가한다 (이제 18 개 항목,
이전 16 개). 다른 모듈은 건드리지 않는다.

| Hex          | 인덱스 | 역할                                          | 노트                                           |
|--------------|-------:|-----------------------------------------------|-----------------------------------------------|
| `#f8b860`    | 16     | 새벽-호박 wisp — 스프린트 트레일 안쪽         | `enemy-hummerwing.js` 에서 **그대로 재사용** (Hummerwing 흉부 새벽-호박 underglow). 트레일이 "따뜻한 스파크가 시원하게 떨어진다" 킬 프레임과 시각적으로 라임; 새 별개 색상 아님. |
| `#a888b0`    | 17     | 옅은 보랏 wisp — 스프린트 트레일 바깥쪽/시원함 | **새 hex.** 프로젝트 전반의 보랏-그림자 외곽선 `#3a2e4a` 의 시원한 형제 (더 옅고, 더 부드럽고, ~30% lift). v0.50.2 에서 새 색상 1 개. |

### 이 선택의 이유

브리프의 "호박/보랏 트레일" 가이던스는 자연스럽게 Phase 2 팔레트의 기존 따뜻함-시원함
화음을 가리켰다. 두 개의 완전히 새 색상을 도입하는 대신:

- **호박 안쪽** 은 `#f8b860` 을 선택했는데 이미 *프로젝트 안에 있고* (Hummerwing) Reed
  의 스프린트 트레일이 flier 의 underglow 와 형제 현상으로 느껴져야 하기 때문 — 잠깐
  공기에 머무는 따뜻함. 이 슬롯에서 새 hex 0 개.
- **보랏 바깥쪽** 은 `#3a2e4a` 를 직접 재사용할 수 없었다 — 그 외곽선은 단단한 실루엣
  외곽이라 반투명 트레일로는 너무 무겁다. 새 옅은-보랏 `#a888b0` 이 같은 색상 family
  에 상당히 더 높은 lightness 로 착륙하고 "외곽선" 이 아닌 "빠르게 사라지는 시원한
  그림자" 로 읽힌다.

### 업데이트된 예산

- Phase 1: 34 hex.
- Phase 2 (v0.50): 새로운 30 hex (39 건드림, 9 공유).
- Phase 2 (v0.50.2): **+1 새 hex** (`#a888b0`). `#f8b860` 은 Hummerwing 에서 그대로
  재사용 — 새 것으로 카운트하지 않음.
- 누적: **65 개 별개 hex.** **Phase 3-4 를 위한 55 색의 여유.**

### `hero-reed.js` 모듈별 팔레트 길이

| 버전     | 길이  | 노트                                              |
|----------|------:|---------------------------------------------------|
| Phase 1  | 13    | (종결)                                             |
| v0.50    | 16    | 해치트 머리/그립 + 그립-그림자용 +3                 |
| v0.50.2  | 18    | 트레일 wisp +2 (하나는 재사용, 하나는 새것)         |

이 문서 상단의 Phase 2 모듈 목록 표는 여전히 `hero-reed.js` 를 16 으로 표시한다 — 그
카운트는 v0.50 baseline 을 반영; v0.50.2 가 여기 명시한 대로 18 로 올린다.

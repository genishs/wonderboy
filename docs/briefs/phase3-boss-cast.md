...
This brief defines **Area 1's boss** — a single end-of-Area encounter spec'd in full for v0.75. The boss fight lives at the end of Stage 4 (`The Old Threshold`, a dark-forest interior) in a small **glade clearing** at the center of the canopy, and is handed off to from `phase3-area1-expansion.md` §8. Boss spec includes silhouette intent, movement FSM, attack pattern, hit reaction, color-mood keywords, animation cue list, arena layout, camera behavior, win/lose conditions, and a tunable parameters table. Numeric quantities are *suggestions* for dev-lead to tune in code.

> **Tribute, not a port.** The boss is **original to this project**. The name (Bracken Warden), the silhouette (a kneeling moss-and-stone guardian rising from rest), the attack (a ground-pulse shockwave), and the arena beat (a small canopy glade) are all invented for the Wonder Boy Tribute project. The "end-of-stage boss fight in a small locked room" beat is a universal action-platformer convention; its execution here uses only original art, original ECS structure, and original audio/timing.

---

## 1. Boss original name — **the Bracken Warden**

The Old Threshold was once watched by something. Long after the people of the Verdant Ruin stepped sideways out of the world, what they posted in their last clearing kept its post. Mosses grew over its shoulders, bracken fronds rooted in its joints, stone joinery slumped into stone joinery — but the **Warden** stayed. It does not chase Reed; it does not hate him; it raises its head only because the wood woke it. **Threat-read: a part of the glade raising up.**
...
A **kneeling humanoid colossus** at the far edge of the glade. ~3 tiles wide × 4 tiles tall when fully risen (in the larger end of the brief's "3-4 tiles per side" range — see §10 for asset dimensions). When at rest (initial state, before Reed enters): the Warden sits hunched-forward, **one knee on the ground, both forearms resting on the bent knee, head bowed**. The silhouette reads as a **moss-covered cairn shape** — a stack of rounded forms, the topmost being the bowed head. The casual viewer's eye should read it as part of the glade's terrain (a forgotten standing stone, a moss-mound between root-knots) until it moves.
...
**Wave visuals.** A short upward burst of moss-and-stone particles at the leading face (1-tile-tall), with a trailing ground-blur that fades over ~10 px behind the leading face. Color: `canopy-shadow-bluegreen` for the moss particles (rhymes with the glade's floor moss), `old-stone amber` for the warm glow at the wave's base (the sigil's family — Reed sees the Warden's stored heat travelling toward him), `velvet-shadow` for the trailing blur. The wave should read distinctly different from any v0.50 entity — there is no hazard or projectile in v0.50 that hugs the ground horizontally.
...
- `moss-shadowed violet` — the violet-grey shade beneath the moss layer where bracken can't reach. Stage 4's `understory-violet` (per area-expansion brief §7); should be reused verbatim. Hex sibling family: `3a2e4a` / `5a4a6e`-adjacent.
...
- `stone-joinery pale` — the visible carved-stone joints (shoulders, elbows, knees). A pale, slightly-cool grey that reads as old standing stone rather than fresh-quarried ruin — the dark forest has been growing around this material for centuries. Reads as **the boss is made of the old wood and what the old wood remembers of stone**.
...
- `moonlight-rim silver` (new in this remap) — a faint, sparing silver rim along the topmost bracken fronds and the moss-cairn crown of the Warden, where the canopy slit overhead catches the bracken in `moonlight-silver`. Reuse the dark-forest decoration palette family verbatim (per area-expansion §7); used only on the Warden's upper silhouette, never on the chest sigil.

The palette should rhyme with the Stage 4 dark-forest tile palette without copying it — the Warden should look **of the same wood and shadow** as the canopy and root-knots, just animated.
...
**Arena dimensions.** ~12 tiles wide × 11 tiles tall. Slightly less than the viewport's 16 × 12 tile space so an HUD strip stays visible at the top of the screen. The arena is a small **dark-forest glade clearing** — a roughly-circular pocket of open ground where the canopy parts overhead into a vertical moonlight column, with gnarled trunks closing the glade on either side and a far-edge wall of root-knot and old standing stone behind the Warden.

```
                          (HUD strip — top 1 tile reserved for lives/vitality)
        ┌───────────────────────────────────────────────────────────┐ (row 0 — out of arena, HUD only)
        ▕                                                            ▕
        ▕   (canopy thinning overhead — single moonlight column      ▕
        ▕    pours down center; gnarled trunks frame the glade on    ▕
        ▕    both sides — visual only, not collision)                ▕
        ▕                                                            ▕
        ▕                                                            ▕
        ▕         (open glade interior — empty floor + canopy)       ▕
        ▕                                                            ▕
        ▕                                                            ▕
        ▕    R (Reed enters here)                              W →   ▕  W = Warden, kneeling, faces left
        ▕    →                                                       ▕
        █████████████████████████████████████████████████████████████  (row 11 — solid glade floor of moss + root-knots)
        col 0                                                     col 11
                                                                       (glade's far-edge wall is solid; Warden cannot move past it; Reed cannot escape past it)
```

**Specifics.**
- **Glade interior:** cols 0-11 (12 tiles wide), rows 0-10 within the arena (11 rows tall). Floor row = row 10 (one tile tall) — moss-on-earth with occasional root-knot detailing baked into the tile, no collision changes.
- **Walls:** the glade's left edge is the **camera-lock boundary**, not a solid tile per se — Reed enters from the anteroom's moonlit path continuously; the camera locks once Reed is fully inside. The glade's far-edge wall (col 11) **is solid**: a 1-tile-thick column at col 11, full 11 rows tall — drawn as a tall mass of standing stone half-swallowed by root and bracken, marking the back of the wood. Not pillars; not ruin masonry. Old stone the forest has grown around.
- **Ceiling:** decorative (drawn as a near-closed canopy with one bright vertical moonlight column descending to row 10 at roughly cols 5-6, illuminating the floor in front of the Warden). Reed cannot reach it with a jump (jump apex ≈ 2.3 tiles ≪ 11 tiles).
- **Reed spawn:** col 1, row 9 (one tile above the floor, one tile in from the left edge). Reed enters facing right.
- **Warden spawn (kneeling pose):** col 9, row 7 (one tile in from the far-edge wall, with the kneeling silhouette occupying ~rows 7-10). Faces left.
- **Camera lock position:** the camera anchors with the glade's left edge at viewport col 0. The viewport is 16 tiles wide; the glade is 12 tiles; the remaining 4 viewport columns to the right show the moonlit-path anteroom's continuation or the canopy extending right, design-lead's call.
...
- Solid far-edge wall = the Warden cannot retreat; the fight ends one way.
- No floor gaps inside the glade = the Warden's shockwave can travel cleanly along the floor without falling into a hole. Spatially clean.
